...
int main() {
    int i;
    printf("Zaid Bin Naveed\n");
    printf("23k-0028\n");
    srand(time(NULL));

    char Name_pool[20][20] = {"Zaid", "Huzaifa", "Bisma", "Abbas",
                              "Dania", "Esha", "Murtaza", "Gulmeena",
                              "Hiba", "Waleed", "Haris", "Kehkashan",
                              "Laiba", "Momina", "Wajeeha", "Yesaullah",
                              "Sabeena", "Tanisha", "Zumer", "Barirah"};
    int Name_Used[20];
    for (i = 0; i < 20; i++) {
        Name_Used[i] = 0;
    }
...
char* Name_Rand(char Names[20][20], int Name_Used[20]) {
    while (1) {
        int random_index = rand() % 20; // Corrected range
...
char* Role_Rand(char Role[5][20], int Role_Used[5]) {
    while (1) {
        int random_index = rand() % 5; // Corrected range
...
int Initialize_Dept(struct Dept dept[], char Names[20][20], int Name_Used[20]) {
    int i;
    char Role[5][20] = {"Director", "Executive", "Manager", "Employee", "Trainee"}; // Corrected size
